...
import { Skeleton } from '@/components/ui/admin/kit/skeleton'
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from '@/components/ui/admin/kit/tooltip'
...
// Create a separate hook for internal sidebar component usage that doesn't throw errors
...
function Sidebar({
  ...
  // Use the internal hook that doesn't throw errors
  const sidebarContext = useSidebarInternal()
  
  // If sidebar context is not available, render a basic sidebar
  if (!sidebarContext) {
    if (collapsible === 'none') {
      return (
        <div
    ...
    // For mobile without context, render a simple div
    ...
    if (isMobile) {
      return (
        <div
          data-slot='sidebar'
          className={cn(
            'bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col md:hidden',
            className
          )}
          {...props}
        >
    ...
    // For desktop without context
    return (
      <div
        data-slot='sidebar'
        className={cn(
          'bg-sidebar text-sidebar-foreground hidden h-full w-(--sidebar-width) flex-col md:flex',
          className
        )}
        {...props}
      >
...
function SidebarTrigger({
  ...
  // If no sidebar context, render a disabled button or nothing
  if (!sidebarContext) {
    return (
      <Button
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  if (!sidebarContext) {
    return null // Don't render rail if no sidebar context
  }
...
function SidebarMenuButton({
  ...
  // Use safe defaults if no context
