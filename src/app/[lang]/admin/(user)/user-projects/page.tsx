...
export default function Projects() {
  ...
  const [manualProjects, setManualProjects] = useState(true);
  const [error, setError] = useState("");
  const [uploadingImages, setUploadingImages] = useState<{ [key: number]: boolean }>({});

  // Hooks

  const { data } = useFetch("get", { endPoint: `/api/resume/projects/` });

  const { mutate: mutateProjects } = useFetch(
    "put",
    { endPoint: `/api/resume/projects/`, body: JSON.stringify({ projects }) },
  ...
  const handleSave = async () => {
    setSaving(true);
    setManualProjects(false);
  };

  const addProject = () =>
    setProjects([...projects, { title: "", description: "", link: "", technologies: [], image: "" }]);
  ...
  useEffect(() => {
    if (!manualProjects) executeMutation();
  }, [manualProjects]);
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-6 sticky top-6">
              ...
              <div className="space-y-4 text-center">
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-800">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">پروژه ثبت شده</p>
                </div>
                <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                  <div className="text-lg font-bold text-green-600 dark:text-green-400">{projects.filter((p) => p.image).length}</div>
                  <p className="text-sm text-green-700 dark:text-green-300 mt-1">پروژه دارای تصویر</p>
                </div>

                {/* Sidebar notes */}
                <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
                  <p className="text-sm text-purple-700 dark:text-purple-300 font-medium">نکات مهم</p>
              ...
              <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-700">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h2 className="text-xl font-bold dark:text-white">مدیریت پروژه‌ها</h2>
                      <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">پروژه‌های خود را اضافه، ویرایش یا حذف کنید</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 md:p-8">
                {projects.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-semibold text-gray-600 dark:text-gray-400 mb-2">هیچ پروژه‌ای ثبت نشده است</h3>
                    <p className="text-gray-500 dark:text-gray-500 text-sm mb-6">برای شروع، اولین پروژه خود را اضافه کنید</p>
                    <button
                      onClick={addProject}
                  ...
                  <div className="space-y-6">
                    {projects.map((project, index) => (
                      <div key={index} className="bg-gray-50 dark:bg-gray-700/50 rounded-2xl border border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-200 overflow-hidden">
                        {/* Single Project */}
                        <div className="p-6">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-lg shadow-lg">{index + 1}</div>
                              <div>
                                <h3 className="text-lg font-semibold text-gray-800 dark:text-white">پروژه {index + 1}</h3>
                                {project.title && <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{project.title}</p>}
                              </div>
                            </div>
                            <button
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                              <div>
                                <label className="block text-sm font-medium mb-2 dark:text-gray-300">عنوان پروژه</label>
                                <input
                                  type="text"
                                  value={project.title}
                              ...
                              <div>
                                <label className="block text-sm font-medium mb-2 dark:text-gray-300">لینک پروژه</label>
                                <input
                                  type="text"
                                  value={project.link}
                              ...
                              <div>
                                <label className="block text-sm font-medium mb-2 dark:text-gray-300">تکنولوژی‌ها</label>
                                <input
                                  type="text"
                            ...
                            <div className="space-y-4">
                              <div>
                                <label className="block text-sm font-medium mb-2 dark:text-gray-300">تصویر پروژه</label>
                                <div className="space-y-3">
                                  {project.image ? (
                                    <div className="relative rounded-xl overflow-hidden border-2 border-gray-300 dark:border-gray-600">
                                      <img src={project.image} alt="Project cover" className="w-full h-48 object-cover" />
                                      <button
                                    ...
                                    >
                                      ...
                                      <p className="text-gray-600 dark:text-gray-400 text-sm">
                                        {uploadingImages[index] ? "در حال آپلود..." : "برای آپلود تصویر کلیک کنید"}
                                      </p>
                                      <p className="text-gray-500 dark:text-gray-500 text-xs mt-1">JPG, PNG, GIF (حداکثر ۲ مگابایت)</p>
                                    </div>
                                  )}
                                </div>
                              </div>
                              <div>
                                <label className="block text-sm font-medium mb-2 dark:text-gray-300">توضیحات پروژه</label>
                                <textarea
